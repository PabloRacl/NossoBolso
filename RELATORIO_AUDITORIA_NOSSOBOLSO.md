...
### 🔴 Prioridade 1: CRÍTICA (Resolver de Imediato)
1. **Erradicar os 4 casts `as any`:**
   - Criar extensão de `Window` para `SpeechRecognition` em `src/types/speech.d.ts`.
   - Tipar os eventos de select em `VehicleModal.tsx` e `UnitPriceCalculatorModal.tsx` usando Type Guards ou `as FuelType` explícito.
2. **Adicionar Cleanups de `setTimeout` nos Modais:**
   - Guardar referências de timers em `useRef<NodeJS.Timeout | null>(null)` e limpar no `return () => clearTimeout(...)` de cada `useEffect`.
3. **Substituir os 18 CSS Inline Proibidos por Tailwind Classes:**
   - Migrar as cores estáticas e bordas de `ThemeSelectorModal`, `FinancialHealthScoreWidget`, `ScoreModal` e `WalletCards` para utilitários Tailwind.

### 🟠 Prioridade 2: ALTA (Design System & Nomenclatura Oficial)
1. **Criar os 3 Componentes Faltantes em `src/components/ui/`:**
   - `Badge.tsx` (com variantes: *Pago*, *Pendente*, *Em Atraso*, *Quitado*, percentuais);
   - `Tabs.tsx` (navegador de abas animado com Framer Motion);
   - `ProgressBar.tsx` (barra de progresso com gradientes oficiais esmeralda/ciano/âmbar).
2. **Migração de Nomenclatura das Pastas para Português:**
   - Renomear gradualmente as pastas de `src/components/` conforme a tabela oficial da Regra 1 do `AGENTS.md` (`auth/` → `autenticacao/`, `debts/` → `dividas/`, `pantry/` → `despensa/`, etc.).

### 🟡 Prioridade 3: MÉDIA (Modularização de Arquivos Monolíticos)
1. **Desmembrar `PantryView.tsx` (1.314 linhas):**
   - Dividir em `PantryTable.tsx`, `PantryShoppingList.tsx`, `PantryCategoryFilter.tsx` e `PantryStats.tsx`.
2. **Desmembrar `AutomotiveView.tsx` (915 linhas):**
   - Separar em `VehicleCard.tsx`, `MaintenanceHistoryTable.tsx` e `FuelEfficiencyWidget.tsx`.
3. **Centralizar Motor Matemático SAC/PRICE:**
   - Extrair toda a matemática de amortização para `src/utils/debtCalculations.ts` e consumir como função pura nos 3 modais.

### 🟢 Prioridade 4: BAIXA (Qualidade Visual e Tokens)
1. **Mapeamento de Cores em Tokens Tailwind:**
   - Mover os 3.608 hexadecimais dispersos para tokens configurados no `tailwind.config.js` ou CSS variables em `index.css`.
