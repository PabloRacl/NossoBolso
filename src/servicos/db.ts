...
import { Transaction, Category, Wallet, Goal, DebtContract, Budget, RecurringTransaction, PantryItem, VehicleRecord, Vehicle, ComponentSpec } from '../tipos';
...
export class NossoBolsoDB extends Dexie {
  ...
  constructor(dbName: string = 'nosso-bolso-db') {
    super(dbName);
...
function getInitialDbName(): string {
  try {
    const data = typeof window !== 'undefined' ? localStorage.getItem('nossobolso_auth_user') : null;
    if (data) {
      const user = JSON.parse(data);
      if (user?.role === 'guest' || user?.id === 'usr_guest') {
        return 'nosso-bolso-guest-demo';
      }
      if (user?.id) {
        return `nosso-bolso-user_${user.id}`;
      }
    }
  } catch {
    // Fallback silencioso
...
let activeDb: NossoBolsoDB = new NossoBolsoDB(getInitialDbName());

// Alterna a base do usuário ativa garantindo isolamento total
export function setDatabaseUser(userId: string | null, isGuest: boolean): NossoBolsoDB {
  const targetDbName = isGuest
    ? 'nosso-bolso-guest-demo'
    : (userId ? `nosso-bolso-user_${userId}` : 'nosso-bolso-guest-demo');

  if (activeDb.name !== targetDbName) {
    try {
      activeDb.close();
    } catch (e) {
      console.warn('Erro ao fechar DB anterior:', e);
    }
    activeDb = new NossoBolsoDB(targetDbName);
  }
  return activeDb;
}

export function getActiveDb(): NossoBolsoDB {
  return activeDb;
}

// Proxy transparente para manter 100% de retrocompatibilidade com componentes e useLiveQuery
export const db: NossoBolsoDB = new Proxy({} as NossoBolsoDB, {
...
// Fechamento automático do DB para upgrade sem conflitos entre abas
activeDb.on('versionchange', () => {
  activeDb.close();
});

// Semente de dados limpos para novos usuários reais cadastrados
export async function seedCleanUserData(database: NossoBolsoDB = activeDb) {
  // 1. Carteiras padrão com saldo R$ 0,00
  ...
  // 2. Categorias padrão essenciais
...
// Semente de dados de demonstração para modo Convidado
export async function seedGuestDemoData(database: NossoBolsoDB = activeDb) {
  const existingWallets = await database.wallets.count();
  if (existingWallets === 0) {
    await generateFullTestDataset();
  }
}

export async function seedInitialData(currentUser?: { id: string; role?: string } | null) {
  if (!currentUser || currentUser.role === 'guest' || currentUser.id === 'usr_guest') {
    setDatabaseUser('usr_guest', true);
    await seedGuestDemoData(activeDb);
  } else {
    setDatabaseUser(currentUser.id, false);
    await seedCleanUserData(activeDb);
  }
}

export async function performSeeding(database: NossoBolsoDB = activeDb) {
  await seedCleanUserData(database);
}
