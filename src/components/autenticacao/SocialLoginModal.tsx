            ...
            >
              ...
              <div className="p-4 rounded-2xl bg-slate-950/80 border border-emerald-500/30 space-y-3">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  type="button"
                  onClick={() => onExecute(socialEmailInput.trim(), socialNameInput.trim())}
                  disabled={loading || !socialEmailInput.trim().includes('@')}
                  className="w-full py-3 px-4 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold rounded-xl shadow-lg shadow-emerald-500/25 transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
                >
                  {loading ? (
                    ...
                    <>
                      <span>
                        Continuar como {socialNameInput.trim().split(' ')[0] || (provider ? provider.toUpperCase() : 'Usuário')}
                      </span>
              ...
              <div className="space-y-3 pt-1">
                <p className="text-xs font-semibold text-slate-400">Ou personalize os dados da conta {provider}:</p>
                <div>
                  <label className="block text-[11px] font-bold text-slate-400 mb-1">E-mail da Conta Social</label>
                  <input
                    type="email"
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-400 mb-1">Nome de Exibição</label>
                  <input
                    type="text"
