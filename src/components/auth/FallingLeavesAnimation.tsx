...
interface TrapLeafState {
  ...
  swayFreq: number;
  swayAmp: number;
  isTrapped: boolean; // Transformed into Prison Cell (Cela de Prisão)
  trapTimer: number;
  stolenText: string | null;
  stolenTextY: number;
  stolenTextOpacity: number;
  prisonerSway: number; // Oscilação/Movimento do prisioneiro tentando escapar
}

export const FallingLeavesAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 1. REGULAR LEAF & MONEY PARTICLES (20 Particles)
    ...
    // 2. FINANCIAL TRAP LEAF (FOLHA ARMADILHA - VIRA CELA DE PRISÃO COM ALGUÉM SE MEXENDO)
    ...
    // DRAW GOLDEN TRAP LEAF (FOLHA DOURADA RELUZENTE)
    ...
    // DRAW ANIMATED PRISON CELL WITH AGITATED PRISONER (CELA DE PRISÃO VIVA)
    const drawPrisonCellWithPrisoner = (
      ...
      // Dark Red / Violet Glow
      c.shadowColor = '#EF4444';
      c.shadowBlur = 25;

      // 1. Prison Cell Interior Wall Background
      c.fillStyle = '#111827';
      ...
      // Brick pattern texture background inside cell
      c.strokeStyle = '#1F2937';
      c.lineWidth = 1;
      for (let y = -h / 2 + 10; y < h / 2 - 10; y += 12) {
        c.beginPath();
        c.moveTo(-w / 2 + 4, y);
        c.lineTo(w / 2 - 4, y);
        c.stroke();
      ...
      // 2. PRISONER ANIMATED INSIDE THE CELL (Mãozinhas segurando as barras e corpo chacoalhando)
      const shakeX = Math.sin(prisonerSway * 1.8) * 3.5;
      const headY = -h * 0.15 + Math.cos(prisonerSway * 2.2) * 2;

      // Prisoner Head (Carinha deseperada no fundo da cela)
      c.fillStyle = '#FCA5A5';
      c.beginPath();
      c.arc(shakeX, headY, size * 0.35, 0, Math.PI * 2);
      c.fill();

      // Expressive Worried/Desperate Eyes (Olhos arregalados)
      c.fillStyle = '#111827';
      c.beginPath();
      ...
      // 3. STEEL PRISON BARS (Grades da Cela em Frente ao Prisioneiro)
      ...
      // Top & Bottom Heavy Iron Beams
      c.fillStyle = '#334155';
      c.fillRect(-w / 2 + 2, -h / 2 + 2, w - 4, 10);
      c.fillRect(-w / 2 + 2, h / 2 - 12, w - 4, 10);

      // Padlock on Cell Gate
      c.fillStyle = '#DC2626';
    ...
    const render = () => {
      ...
      // Check if it's time to spawn Trap Leaf
      if (!trapLeaf.active && now > nextTrapTime) {
        spawnTrapLeaf();
      ...
      particles.forEach((p) => {
        ...
        const maxWindDist = 200;

        if (dist < maxWindDist) {
          const force = (1 - dist / maxWindDist) * (p.depth === 'fg' ? 10 : 6);
        ...
        if (p.morphProgress < 0.5) {
          ...
          drawPremiumMoney(ctx, p.size, flipScale);
        }

        ctx.restore();
      });

      // RENDER FINANCIAL TRAP LEAF / PRISON CELL (CELA COM PRISIONEIRO SE MEXENDO)
      if (trapLeaf.active) {
        ...
        trapLeaf.prisonerSway += 0.15; // Velocidade de animação do prisioneiro

        // Hover Detection: Mouse touching Golden Trap Leaf turns it into a Prison Cell!
        ...
        // Draw Trap Leaf or Animated Prison Cell
        ctx.save();
        ...
        // Handle Trapped Cell State (Flutuante de texto de prejuízo)
        if (trapLeaf.isTrapped) {
          trapLeaf.trapTimer++;
          ...
          if (trapLeaf.trapTimer > 50) {
            trapLeaf.active = false; // Cell dissolves after animation!
          }
        }
